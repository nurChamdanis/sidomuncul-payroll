...
class CoreController extends BaseController{
     ...
     * @return Response
     * ----------------------------------------------------
     * name: responseSuccess(data, message, code)
     * desc: function for set default format response success
     */
    public function responseSuccess(?array $data, string $message, int $code = 200) : Response
    {
        if(!is_array($data)){
            $data = array(
                'status'    => true,
     ...
     * @return Response
     * ----------------------------------------------------
     * name: responseError(data, message, code)
     * desc: function for set default format response error
     */
    public function responseError(?array $data, string $message, int $code = 500) : Response
    {
        if(!is_array($data)){
            $data = array(
                'status'    => false,
     ...
     * @return Response
     * ----------------------------------------------------
     * name: loadView(view, data)
     * desc: function for set default format response error
     */
    public function loadView(string $view, array $data = array()) : string
    {
