...
$(function(){
    OptionsSelect({id: 'company_id', url: 'master_tunjangan/options/company', payload: {}});
    
    const additionalOptions = {
        scrollX: true,
        scrollCollapse: true,
        createdRow: function (row, data, dataIndex) {
            const dataRow = data[20].split("~");
    ...
    dataTableTemporaryLoader = new DataTableLoader('tableImportExcel', 'master_tunjangan/datatable_uploaded', {...tableTempFilters()}, additionalOptions);
    dataTableTemporaryLoader.setColumnDefs([
        { 
            targets: 0, 
            width: '80px', 
            render: (data, type, row) => {
                var onclick = data == '0' ? `onclick="showModalInvalid('${row[19]}')"` : '';
                if(data == '0') {	
                    invalid++;
                }
        ...
        { targets: 15, render: (data) => `<div class="author">${data}</div>`, name: 'created_by'},
        { targets: 16, render: (data) => `<div class="author">${data}</div>`, name: 'changed_by'},
        { targets: 17, render: (data) => `<div class="timestamp">${data}</div>`, name: 'created_dt'},
        { targets: 18, render: (data) => `<div class="timestamp">${data}</div>`, name: 'changed_dt'},
        { targets: 19, name: 'error_message', visible: false},
        { targets: 20, name: 'rowstatus', visible: false, orderable: false},
    ]);
...
async function uploadExcelTemplate(){
    ...
    $(`#wrapperImportExcel`).hide();
        ...
        try {
            ...
            const response = await fetch(SITE_URL + 'master_tunjangan/upload_template/read', config);
...
function downloadExcelTemplate()
{
    ...
    $.fileDownload(`${SITE_URL}master_tunjangan/download_template/excel`, {
        httpMethod: "POST",
        data:  {
...
async function toggleInvalid()
{
    ...
    const response = await fetch(SITE_URL + 'master_tunjangan/upload_template/get-invalid', config);
    const json = await response.json();
...
async function handleSubmitImport()
{
    ...
    const response = await fetch(SITE_URL + 'master_tunjangan/upload_template/create', config);
    const json = await response.json();
