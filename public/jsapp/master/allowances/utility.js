...
async function loadAllowanceArea()
{
    ...
    const response = await fetch(`${SITE_URL}master_tunjangan/area?company_id=${companyid}`);
...
async function loadAllowanceAreaGrup()
{
    ...
    const companyid = companyId.val() ?? '';
    const response = await fetch(`${SITE_URL}master_tunjangan/area_grup?company_id=${companyid}`);
    const result = await response.json();
    const enabled = segment.val() == 'id' ?  'disabled' : '';
    const html = result.data.map(function(item){
...
async function loadAllowanceRules()
{
    ...
    const companyid = companyId.val() ?? '';
    const response = await fetch(`${SITE_URL}master_tunjangan/payroll_rules?company_id=${companyid}`);
    const result = await response.json();
    const enabled = segment.val() == 'id' ?  'disabled' : '';
    const html = result.data.map(function(item){
